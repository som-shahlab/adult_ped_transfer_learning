...
if __name__ == "__main__":
	...
	df=og_df.query("fold_id==['val','test'] | adult_at_admission==1")[['person_id','admit_date','discharge_date']]
	df = df[df['person_id'] != 86281596]
	df['date']=pd.to_datetime(df['admit_date']).dt.date
	...
	df=og_df.query("fold_id==['val','test'] | adult_at_admission==0")[['person_id','admit_date','discharge_date']]
	df = df[~df['person_id'].isin([86281596,72463221, 31542622, 30046470])]
	df['date']=pd.to_datetime(df['admit_date']).dt.date
